                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            @if($item->product_image)
                                                <img src="{{ Storage::url($item->product_image) }}" style="width:40px;height:40px;object-fit:cover;border-radius:6px;">
                                            @endif
                                            <div>
...
<script>
    document.querySelector('[name="status"]')?.addEventListener('change', function () {
        document.getElementById('shippingFields').style.display = this.value === 'shipped' ? 'block' : 'none';
    });
</script>
@endpush
